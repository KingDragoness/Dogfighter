...
namespace MFlight.Demo
{
    ...
    {
        [Header("Components")]
        [SerializeField] public MouseFlightController controller = null;

        [Header("Physics")]
        public float accelerationThrust = 15f;
        public float maximumThrust = 140f;
        ...
        private void Start()
        {
            ...
            if (controller == null)
            {
                controller = FindObjectOfType<MouseFlightController>();
                //Debug.LogError(name + ": Plane - Missing reference to MouseFlightController!");
            }

            if (controller == null)
            {
                controller = MouseFlightController.instance;
                Debug.LogError(controller + ": Plane - Missing reference to MouseFlightController!");
            }
        ...
        private void Update()
        {
            ...
            float speedAdjustment = Input.GetAxis("Throttle");
            thrust += speedAdjustment * accelerationThrust;
            ...
            float keyboardRoll = Input.GetAxis("HorizontalAlt");
            if (Mathf.Abs(keyboardRoll) > .25f)
            {
                rollOverride = true;
            }

            float keyboardPitch = Input.GetAxis("VerticalAlt");
